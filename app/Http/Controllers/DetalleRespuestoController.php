...
class DetalleRespuestoController extends Controller
{
    ...
    {
        $equipos = DB::select("SELECT marca, equipo, nombre_repuesto, cantidad_pedido, fecha_solicitud, inventarios.id
    ...
    {
        $hola = DB::select("SELECT  inventarios.id as idinventario, COUNT(inventarios.id) as total_respuesto_solicitado, marca, equipo, serial, modelo,centro_salud
        FROM detalle_requirimientos
        LEFT JOIN inventarios ON inventario_id = inventarios.id
